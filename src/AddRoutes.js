...
const AddRoutes = () => {
  ...
  const [routes, setRoutes] = useState(Array.from({ length: 5 }, () => ({ route_name: '', lat: '', lang: '', isValid: false }))); // Add an empty row

  const validateRow = (route) => {
    return (route.route_name.trim() !== "" && route.lat.trim() !== "" && route.lang.trim() !== "") 
  };
  ...
  const updateRouteDetails = (index, field, value) => {
    setRoutes(prevRoutes => {
      const updatedRoutes = prevRoutes.map((route, i) =>
        i === index
          ? { ...route, [field]: value, order: i + 1, isValid: validateRow({ ...route, [field]: value }) }
          : route
      );
  
      // Check if editing last row and add a new empty one
      if (index === updatedRoutes.length - 1 && value !== '') {
        return [...updatedRoutes, { route_name: '', lat: '', lang: '', isValid: false }];
      }
...
const handleSubmit = async (e) => {
  ...
  const hasPartiallyFilled = routes.some(
    route => validateRow(route) === false && 
             (route.route_name.trim() !== "" || route.lat.trim() !== "" || route.lang.trim() !== "")
  );

  if (hasPartiallyFilled) {
    alert("Please fill in complete details for all subroutes or leave them empty.");
    return;
  }
  ...
  return (
    <div className="container mt-14">
      ...
      <div className="flex justify-end mr-52">
        ...
        <button class="btn btn-primary bg-[#062e61] text-white font-bold py-2 px-4 mb-6 rounded-lg" onClick={toggleModal}>
          Add Routes
        </button>
      </div>
                  ...
                  <div className="add-route">
                    <table className="tabs">
                      <thead className=" h-10 bg-[#062e61] text-white">
                        <tr className="">
                          <th className="text-center uppercase font-semibold text-sm">route_name</th>
                      ...
                      <tbody>
                      {routes.map((route = { route_name: '', lat: '', lang: '' }, index) => (
                          <tr key={index}>
